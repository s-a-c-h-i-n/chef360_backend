...
def get_dev_conn():
    print(f'***** Dev Connect {connection_string_dev=} \n')
    connection_string = connection_string_dev.format(drv = driver, svr = server, uid = username, pwd = password)
    conn = pyodbc.connect(connection_string)
    return conn

# Use Azure managed identity
def get_azmi_conn():
    print(f'***** Prod Connect {connection_string_azmi=} \n')
    credential = DefaultAzureCredential(managed_identity_client_id=client_id)
    token_bytes = credential.get_token("https://database.windows.net/.default").token.encode("UTF-16-LE")
    token_struct = struct.pack(f'<I{len(token_bytes)}s', len(token_bytes), token_bytes)
    SQL_COPT_SS_ACCESS_TOKEN = 1256  # This connection option is defined by microsoft in msodbcsql.h
    
    connection_string = connection_string_azmi.format(drv = driver, svr = server)
